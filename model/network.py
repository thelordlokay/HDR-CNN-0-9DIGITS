...
from model.layers import Convolution2D
from model.layers import Maxpooling2D
from time import *
...
class Net:
    ...
    def train(self, training_data, training_label, batch_size, epoch, weights_file):
        ...
        for e in range(epoch):
            for batch_index in range(0, training_data.shape[0], batch_size):
                ...
                # 13. Time Calculation: It records the end time of the current batch iteration and calculates the time taken for the iteration
                end_time = time()
                batch_time = end_time-start_time
                # 14. Remaining Time Estimation: It estimates the remaining time for the epoch to complete based on the time taken for the current batch iteration.
        ...
        # 16. Weights Dumping: After completing all epochs and batches, it extracts the weights and biases from each layer of the network and dumps them into a file specified by weights_file using pickle for later use in testing or inference.
        obj = []
    ...
    # The test method is responsible for evaluating the performance of the neural network on the given test data and labels.
    ...
    # Set up progress bar visualization.
        ...
        # Calculate the step size to update the progress bar.
        step = float(test_size) / float(toolbar_width)
        st = 1
        total_acc = 0

            # Iterate over each data sample in the test set.
        for i in range(test_size):
            ...
            # Perform a forward pass through the network layers to get the output.
        for l in range(self.lay_num):
            output = self.layers[l].forward(x)
            x = output

        # Check if the predicted class matches the true class and update the total accuracy.
        if np.argmax(output) == np.argmax(y):
            total_acc += 1

            # Complete the progress bar visualization.
        sys.stdout.write("\n")

            # Print the test results including the test size and accuracy.
        print('=== Test Size:{0:d} === Test Acc:{1:.2f} ==='.format(test_size, float(total_acc) / float(test_size)))



    # The test_with_pretrained_weights method evaluates the performance of the neural network using pretrained weights.
    ...
    def test_with_pretrained_weights(self, data, label, test_size, weights_file):
        ...
        # Set the pretrained weights for each layer in the network.
        ...
        self.layers[11].feed(b[11]['fc7.weights'], b[11]['fc7.bias'])
            ...
            # Set up the progress bar visualization.
        toolbar_width = 40
        sys.stdout.write("[%s]" % (" " * (toolbar_width - 1)))
        sys.stdout.flush()
        sys.stdout.write("\b" * (toolbar_width))
    ...
    # The predict_with_pretrained_weights method predicts the class of the input data using pretrained weights.
    def predict_with_pretrained_weights(self, inputs, weights_file):
        ...
        # Determine the predicted digit and its probability.
